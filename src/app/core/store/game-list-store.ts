...
export class GameListStore {
    ...
    searchRecentlyPlayedGames() {
        console.info('Searching recently played games...');
        this._state.update(s => ({...s, loadingStatus: LoadingStatus.LOADING}));
        this._gameService.searchRecentGames(this._state().pageNumber, this._state().pageSize).subscribe({
            next: result => {
    ...
    loadMore() {
        ...
        this.searchRecentlyPlayedGames();
    }
}
